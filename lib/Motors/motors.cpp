...
void Motors::MoveBaseInDirection(int degree, uint8_t speed)
{
    ...
    if (m1 >= 0) upper_left_motor_.MoveForward(); else upper_left_motor_.MoveBackward();
    if (m2 >= 0) lower_center_motor_.MoveForward(); else lower_center_motor_.MoveBackward();
    if (m3 >= 0) upper_right_motor_.MoveForward(); else upper_right_motor_.MoveBackward();
};
