...
void loop() {
 ...
 // Ajustar velocidad según el ángulo
        motors.MoveOmnidirectionalBase(ballAngle,0.3, 0, kCorrectionDegreeOffset);
    } else {
        ...
        if(targetGoalData.signature == targetSignature) {
            motors.MoveOmnidirectionalBase(targetGoalData.cameraAngle, 0.3, 0, kCorrectionDegreeOffset);
        }
    }
}